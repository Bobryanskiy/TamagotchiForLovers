...
//    }
    ...
    suspend fun feedPetSimple(petState: PetState?, pairId: String, isOnline: Boolean): PetState? {
        return petState?.apply {
            hunger -= 4
            hunger = hunger.coerceIn(0..100)
            lastUpdateTime = System.currentTimeMillis()
            if (!isOnline) saveToSharedPreferences(this)
            else {
                firestore.collection("pairs").document(pairId).update("petState.hunger", hunger).await()
                firestore.collection("pairs").document(pairId).update("petState.lastUpdateTime", lastUpdateTime).await()
            }
        }
    }
}
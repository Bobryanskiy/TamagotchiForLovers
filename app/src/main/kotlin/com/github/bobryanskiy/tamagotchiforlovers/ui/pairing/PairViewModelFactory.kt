...
class PairViewModelFactory(private val context: Context) : ViewModelProvider.Factory {
    ...
    override fun <T : ViewModel> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(PairViewModel::class.java)) {
            return PairViewModel(repository = PairRepository(pairStorage = PairStorage(context = context.applicationContext), petStorage = PetStorage(context.applicationContext), dataSource = PairViaFirebase())) as T
        }
